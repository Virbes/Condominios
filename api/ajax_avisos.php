...
$qry = @mysqli_query($link, "   SELECT a.* FROM alerta a 
								WHERE a.cve_sysusuario='" . $_SESSION["CVE_SYSUSUARIO"] . "'
								AND a.cve_alerta='" . $IDPOST . "'" . $WHERE_SELECT);
...
if ($MODPOST ==  $idpm . '_estado_save') {
	...
	if ($num > 0) {
		...
		$qryString =     " 	UPDATE alerta a SET 
								cve_statuscat		=	'" . addslashes($status) . "'
							WHERE  	cve_alerta	=	'" . $IDPOST . "'
...
if ($_POST["draw"]) {
	...
	" AND (
						UPPER(a.cve_alerta) LIKE UPPER('%" . $_POST["search"]["value"] . "%') 
	...
	//$where .= isset($_POST["order"]) ? " ORDER BY " . $_POST["order"]["0"]["column"] . ' ' . $_POST["order"]["0"]["dir"] : " ORDER BY a.cve_alerta DESC";

	$where .= ' ORDER BY a.cve_alerta DESC ' . ($_POST["length"] != -1 ? ' LIMIT ' . $_POST["start"] . ',' . $_POST["length"] : '');
	...
	if ($num_rows > 0) {
		while ($row = @mysqli_fetch_array($qry)) {
			...
			array_push($array_buttons, ["info", "Editar", "fas fa-edit", "", "Editar alerta", "2", $idpm . "_editar", $fileaction]);
