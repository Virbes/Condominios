...
$qry = @mysqli_query($link, "   SELECT a.* FROM areascomunes a 
								WHERE a.cve_sysusuario='" . $_SESSION["CVE_SYSUSUARIO"] . "'
								AND a.cve_areascomunes='" . $IDPOST . "'" . $WHERE_SELECT);
...
if ($MODPOST ==  $idpm . '_estado_save') {
	...
	if ($num > 0) {
		...
		$qryString =     " 	UPDATE areascomunes a SET 
								cve_statuscat		=	'" . addslashes($status) . "'
							WHERE  	cve_areascomunes	=	'" . $IDPOST . "'
...
if ($MODPOST ==  $idpm . '_editar_save') {
	...
	if ($num > 0) {
		...
		$qryString =     " 	INSERT INTO areascomunes(
												fecha_sys, 
												cve_statuscat,
												cve_sysusuario,
												cve_condominio,
...
if ($_POST["draw"]) {
	...
	" AND (
						UPPER(a.cve_areascomunes) LIKE UPPER('%" . $_POST["search"]["value"] . "%') 
	...
	//$where .= isset($_POST["order"]) ? " ORDER BY " . $_POST["order"]["0"]["column"] . ' ' . $_POST["order"]["0"]["dir"] : " ORDER BY a.cve_areascomunes DESC";

	$where .= ' ORDER BY a.cve_areascomunes DESC ' . ($_POST["length"] != -1 ? ' LIMIT ' . $_POST["start"] . ',' . $_POST["length"] : '');
	...
	if ($num_rows > 0) {
		while ($row = @mysqli_fetch_array($qry)) {
			...
			array_push($array_buttons, ["info", "Editar", "fas fa-edit", "", "Editar areascomunes", "2", $idpm . "_editar", $fileaction]);
