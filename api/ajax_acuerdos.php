...
$qry = @mysqli_query($link, "   SELECT a.* FROM acuerdosasamblea a 
								WHERE a.cve_sysusuario='" . $_SESSION["CVE_SYSUSUARIO"] . "'
								AND a.cve_acuerdosasamblea='" . $IDPOST . "'" . $WHERE_SELECT);
...
if ($MODPOST ==  $idpm . '_estado_save') {
	...
	if ($num > 0) {
		...
		$qryString =     " 	UPDATE acuerdosasamblea a SET 
								cve_statuscat		=	'" . addslashes($status) . "'
							WHERE  	cve_acuerdosasamblea	=	'" . $IDPOST . "'
...
if ($MODPOST ==  $idpm . '_editar_save') {
	...
	if ($num > 0) {
		...
		$qryString =     " 	INSERT INTO acuerdosasamblea(
												fecha_sys, 
												cve_statuscat,
												cve_sysusuario,
												cve_condominio,
...
if ($_POST["draw"]) {
	...
	" AND (
						UPPER(a.cve_acuerdosasamblea) LIKE UPPER('%" . $_POST["search"]["value"] . "%') 
						OR 	UPPER(a.descripcion) LIKE UPPER('%" . $_POST["search"]["value"] . "%')
					)";

	$qrystring = "	SELECT 
						a.*
					FROM acuerdosasamblea a
					WHERE a.cve_statuscat IN (1,2)
					AND a.cve_sysusuario='" . $_SESSION["CVE_SYSUSUARIO"] . "' AND a.cve_condominio='" . $_SESSION["CONDOMINIO"]["cve_condominio"] . "'"  . $where;
	...
	//$where .= isset($_POST["order"]) ? " ORDER BY " . $_POST["order"]["0"]["column"] . ' ' . $_POST["order"]["0"]["dir"] : " ORDER BY a.cve_acuerdosasamblea DESC";

	$where .= ' ORDER BY a.cve_acuerdosasamblea DESC ' . ($_POST["length"] != -1 ? ' LIMIT ' . $_POST["start"] . ',' . $_POST["length"] : '');
	...
	if ($num_rows > 0) {
		while ($row = @mysqli_fetch_array($qry)) {
			...
			array_push($array_buttons, ["info", "Editar", "fas fa-edit", "", "Editar acuerdosasamblea", "2", $idpm . "_editar", $fileaction]);
