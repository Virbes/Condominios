...
$qry = @mysqli_query($link, "   SELECT a.* FROM cobro a 
								WHERE a.cve_sysusuario='" . $_SESSION["CVE_SYSUSUARIO"] . "'
								AND a.cve_cobro='" . $IDPOST . "'" . $WHERE_SELECT);
...
if ($MODPOST ==  $idpm . '_estado_save') {
	...
	if ($num > 0) {
		...
		$qryString =     " 	UPDATE cobro a SET 
								cve_statuscat		=	'" . addslashes($status) . "'
							WHERE  	cve_cobro	=	'" . $IDPOST . "'
...
if ($_POST["draw"]) {
	...
	" AND (
						UPPER(a.cve_cobro) LIKE UPPER('%" . $_POST["search"]["value"] . "%') 
	...
	//$where .= isset($_POST["order"]) ? " ORDER BY " . $_POST["order"]["0"]["column"] . ' ' . $_POST["order"]["0"]["dir"] : " ORDER BY a.cve_cobro DESC";

	$where .= ' ORDER BY a.cve_cobro DESC ' . ($_POST["length"] != -1 ? ' LIMIT ' . $_POST["start"] . ',' . $_POST["length"] : '');
	...
	if ($num_rows > 0) {
		while ($row = @mysqli_fetch_array($qry)) {
			...
			array_push($array_buttons, ["info", "Editar", "fas fa-edit", "", "Editar cobro", "2", $idpm . "_editar", $fileaction]);
